...
from src.monthly import render_monthly, load_latest, run_update, today_local
import numpy as np
import pandas as pd
...
APP_VERSION = 'V11.20'
APP_SUBTITLE = 'Pronóstico hidrológico multivariable · caudal + lluvia + propagación + escenarios históricos'
FORECAST_DAYS = 60
...
def normalize_frame(df):
    ...
    if 'datetime' in result.columns:
        result['datetime'] = naive_datetime(result['datetime'])
        result = (
...
def fmt_delta_level(value):
    ...
    arrow = '↑' if value > 0.01 else '↓' if value < -0.01 else '→'
...
def delta_days(df, column, days):
    ...
    return float(values.iloc[-1] - values.iloc[-(days + 1)])
...
def prepare_sn_observed(df):
    ...
    level_col = None
    for candidate in ['nivel', 'value', 'nivel_san_nicolas']:
    ...
    result = df[['datetime', level_col]].copy()
    result['nivel'] = numeric(result[level_col])
    result = (
        result.dropna(subset=['datetime', 'nivel'])
        .sort_values('datetime')
        .reset_index(drop=True)
    )
    return result[['datetime', 'nivel']]
...
st.markdown(
    ...
    .st-key-rio_navigation button p {
        font-size: 1rem;
        font-weight: 600;
        white-space: normal;
        text-align: left;
    ...
    .st-key-rio_navigation button[kind="primary"] {
        background-color: #164b83 !important;
    ...
    .st-key-rio_navigation button:focus-visible {
...
if 'rio_view' not in st.session_state:
    st.session_state['rio_view'] = 'monthly'


def select_rio_view(view):
    st.session_state['rio_view'] = view


with st.container(key='rio_navigation'):
    nav_month, nav_full = st.columns(2)
    ...
    with nav_month:
        st.button(
            ...
            key='rio_monthly_button',
            on_click=select_rio_view,
            args=('monthly',),
        )
...
with st.sidebar:
    ...
    st.caption(
        'El cálculo se ejecuta con la tarea programada. '
        'Para ejecutar el control nuevamente, usá Actions → '
        'Actualizar Rio Parana en GitHub.'
    )

    st.divider()
    st.caption('Horizonte')
...
try:
    ...
    st.warning(f'No se pudo leer el resultado guardado: {exc}')
...
if not st.session_state.get('data_loaded', False):
    ...
    if error:
        ...
        st.info(
            ...
            'Ejecutá Actions → Actualizar Rio Parana → Run workflow en GitHub.'
        )

    st.stop()

sn_history = st.session_state['sn_history']
upstream_history = st.session_state['upstream_history']
upstream_meta = st.session_state['upstream_meta']
exog_history = st.session_state['exog_history']
exog_future = st.session_state['exog_future']
exog_meta = st.session_state['exog_meta']
hydrology = st.session_state['hydrology']
models = st.session_state['models']
metrics = st.session_state['metrics']
forecast = st.session_state['forecast']
base_ts = st.session_state['base_date']

visible_start = base_ts - pd.Timedelta(days=visible_days)

st.success(
    'Mostrando el último resultado guardado. Base: '
    + pd.Timestamp(base_ts).strftime('%d/%m/%Y')
)

current_level = current_value(sn_history, 'nivel')
delta_1 = delta_days(sn_history, 'nivel', 1)
delta_7 = delta_days(sn_history, 'nivel', 7)
current_flow = current_value(exog_history, 'caudal_m3s')
flow_station = exog_meta.get('main_flow_station')
flow_status = flow_state(exog_history)
hydro_estimate = hydrology.get('current_estimate', {})
delay = hydro_estimate.get('delay_days')
delay_min = hydro_estimate.get('delay_min')
delay_max = hydro_estimate.get('delay_max')
correlation = safe_float(hydro_estimate.get('correlation'))
...
with c3:
    st.metric('Caudal', fmt_flow(current_flow), flow_status)

    if flow_station:
        st.caption(f'Serie de referencia: {flow_station}')
...
with c4:
    if delay is not None:
        ...
        if np.isfinite(correlation):
            st.caption(f'Correlación anual robusta: {correlation:.2f}')
    else:
        st.metric('Demora Corrientes → SN', '—')

response_ratio = safe_float(hydro_estimate.get('response_m_per_m'))
corrientes_change_7d = safe_float(hydro_estimate.get('corrientes_change_7d'))
expected_sn_change = safe_float(hydro_estimate.get('expected_sn_change'))

if np.isfinite(response_ratio) or np.isfinite(corrientes_change_7d):
    ...
    with t2:
        st.metric(
            ...
            f'{response_ratio:.2f} m/m' if np.isfinite(response_ratio) else '—',
        )
    ...
    st.caption(
        ...
        'no sobre su altura absoluta. El efecto se ubica temporalmente '
        'dentro del rango histórico de propagación mostrado arriba.'
    )
...
if 'lower' in forecast.columns and 'upper' in forecast.columns:
    ...
    valid_uncertainty = upper.notna() & lower.notna()
    ...
    if valid_uncertainty.any():
        fig.add_trace(
            go.Scatter(
                ...
                y=upper,
                mode='lines',
                line=dict(width=0, color='rgba(148,163,184,0.0)'),
                showlegend=False,
                hoverinfo='skip',
            )
        )
...
if (
    ...
    and numeric(forecast['scenario_adverse']).notna().any()
):
    fig.add_trace(
        go.Scatter(
            x=forecast['datetime'],
...
if (
    ...
    and numeric(forecast['scenario_extreme']).notna().any()
):
    fig.add_trace(
        go.Scatter(
            x=forecast['datetime'],
            ...
            line=dict(color='#ef4444', width=3.5, dash='dot'),
        )
    )
...
st.caption(
    'Los escenarios adverso y extremo representan eventos históricos '
    'comparables y no una predicción meteorológica determinística a 60 días.'
)

st.subheader('Horizontes del pronóstico')
horizon_columns = st.columns(4)

for column, day in zip(horizon_columns, [15, 30, 45, 60]):
    ...
    with column:
        ...
        st.metric(
            ...
            (
                ...
                if np.isfinite(prediction) and np.isfinite(current_level)
                else None
            ),
        )
    ...
    scenario_rows.append(
        {
            ...
            'Pronóstico central [m]': safe_float(row.get('prediction')),
            'Probable histórico [m]': safe_float(row.get('scenario_probable')),
            'Adverso [m]': safe_float(row.get('scenario_adverse')),
            'Extremo histórico [m]': safe_float(row.get('scenario_extreme')),
        }
    )
...
for station in STATIONS:
    ...
    if station == 'San Nicolás':
        ...
        level_value = current_value(sn_history, 'nivel')
        d1 = delta_days(sn_history, 'nivel', 1)
        d7 = delta_days(sn_history, 'nivel', 7)
    else:
        level_history = upstream_history
        level_value = current_value(upstream_history, level_col)
        d1 = delta_days(upstream_history, level_col, 1)
        d7 = delta_days(upstream_history, level_col, 7)

    flow_value = current_value(exog_history, flow_col)
    rain_7 = np.nan

    if rain_col in exog_history.columns:
        rain_values = numeric(exog_history[rain_col]).fillna(0.0).tail(7)
        rain_7 = float(rain_values.sum())
...
for station in STATIONS:
    ...
    if not exog_history.empty and source_col in exog_history.columns:
        valid = exog_history[
            ['datetime', col, source_col]
            + ([quality_col] if quality_col in exog_history.columns else [])
        ].copy()
        ...
        if not valid.empty:
            last = valid.iloc[-1]
            source = str(last.get(source_col, 'desconocido'))

            if quality_col in valid.columns:
                quality = safe_float(last.get(quality_col))
...
flow_quality_table = pd.DataFrame(flow_quality_rows)
...
st.caption(
    'Calidad 100% = observación directa. Valores interpolados o '
    'reconstruidos tienen menor calidad y reciben menor peso en el modelo.'
)
...
for station in STATIONS:
    ...
    if (
        ...
        and numeric(exog_history[col]).notna().any()
    ):
        available_flow_stations.append(station)

if available_flow_stations:
    default_flow_index = 0

    if flow_station in available_flow_stations:
        default_flow_index = available_flow_stations.index(flow_station)
    ...
    selected_flow_col = FLOW_COLUMNS[selected_flow_station]
    ...
    flow_hist[selected_flow_col] = numeric(flow_hist[selected_flow_col])
    q_current = current_value(exog_history, selected_flow_col)
    q_delta_7 = delta_days(exog_history, selected_flow_col, 7)
    q_future = exog_future.copy()

    if selected_flow_col in q_future.columns:
        q_future[selected_flow_col] = numeric(q_future[selected_flow_col])
    ...
    if not q_future.empty and selected_flow_col in q_future.columns:
        future_values = q_future[selected_flow_col]

        if len(future_values) >= 15:
            q15 = safe_float(future_values.iloc[14])
            qmax15 = safe_float(future_values.iloc[:15].max())
    ...
    if selected_source_col in exog_history.columns:
        source_rows = exog_history[
            [selected_flow_col, selected_source_col]
            + (
        ...
        source_rows = source_rows.dropna(subset=[selected_flow_col])

        if not source_rows.empty:
            last_row = source_rows.iloc[-1]
            last_source = str(last_row.get(selected_source_col, 'desconocido'))

            if selected_quality_col in source_rows.columns:
                last_quality = safe_float(last_row.get(selected_quality_col))
    ...
    with fm1:
        st.metric(
            'Caudal actual',
            fmt_flow(q_current),
            fmt_flow(q_delta_7) if np.isfinite(q_delta_7) else None,
        )
        st.caption(
    ...
    if selected_source_col in flow_hist.columns:
        ...
        reconstructed_mask = (
            flow_hist[selected_flow_col].notna() & ~observed_mask
        )

        observed_y = flow_hist[selected_flow_col].where(observed_mask)
        reconstructed_y = flow_hist[selected_flow_col].where(
            reconstructed_mask
        )
    ...
    if selected_flow_col in exog_future.columns and not exog_future.empty:
    ...
    if rain_col in exog_future.columns:
        ...
        if 'rain_forecast_available' in rain_future_plot.columns:
            rain_future_plot = rain_future_plot[
                rain_future_plot['rain_forecast_available'].fillna(False)
            ]
        else:
...
with prop3:
    st.metric(
        'Correlación',
        f'{correlation:.2f}' if np.isfinite(correlation) else '—',
    )

with prop4:
    similar_count = hydro_estimate.get('similar_event_count', 0)
    st.metric('Eventos similares', str(similar_count))
...
if 'nivel_corrientes' in upstream_history.columns:
    ...
    sn_compare = sn_history[['datetime', 'nivel']].copy()
    compare = corrientes.merge(sn_compare, on='datetime', how='inner')
    compare['nivel_corrientes'] = numeric(compare['nivel_corrientes'])
    compare['nivel'] = numeric(compare['nivel'])
    compare = compare.dropna()

    if len(compare) >= 30:
        corr_med = compare['nivel_corrientes'].rolling(
            30, min_periods=10
        ).median()

        sn_med = compare['nivel'].rolling(
            30, min_periods=10
        ).median()

        corr_std = compare['nivel_corrientes'].rolling(
            60, min_periods=20
        ).std()

        sn_std = compare['nivel'].rolling(
            60, min_periods=20
        ).std()
        ...
        st.caption(
            'La comparación está normalizada porque Corrientes y '
            'San Nicolás utilizan ceros hidrométricos diferentes.'
        )

st.subheader('Eventos históricos comparables')
similar_events = hydrology.get('similar_events')

if isinstance(similar_events, pd.DataFrame) and not similar_events.empty:
    event_columns = [
        col
        for col in [
            'start_date',
        ...
        if col in similar_events.columns
    ]

    events_display = similar_events[event_columns].head(15).copy()
...
years_available = sorted(
    sn_history['datetime'].dt.year.dropna().astype(int).unique().tolist(),
    reverse=True,
)
...
if years_available:
    ...
    default_years = [
        year for year in years_available if year >= current_year - 4
    ][:5]
    ...
    if selected_years:
        yoy_fig = go.Figure()
        start_month = comparison_start.month
        start_day = comparison_start.day
        end_month = comparison_end.month
        end_day = comparison_end.day
        ...
        for year in selected_years:
            ...
            md = x['datetime'].dt.month * 100 + x['datetime'].dt.day
            ...
            x['comparison_day'] = np.arange(1, len(x) + 1)
...
st.subheader('Corrientes vs San Nicolás · propagación año por año')

corrientes_yearly = hydrology.get('corrientes_yearly', pd.DataFrame())
corrientes_robust = hydrology.get('corrientes_robust', {})
hydro_dataset = hydrology.get('dataset', pd.DataFrame())
...
if (
    ...
    annual_display = annual_display.sort_values('year', ascending=False)
    ...
    with a3:
        robust_corr = safe_float(corrientes_robust.get('correlation'))

        st.metric(
            'Correlación mediana',
            f'{robust_corr:.2f}' if np.isfinite(robust_corr) else '—',
        )
    ...
    years_corr = annual_display['year'].dropna().astype(int).tolist()
    ...
    if (
        ...
        annual = hydro_dataset[
            pd.to_datetime(
                hydro_dataset['datetime'], errors='coerce'
            ).dt.year == selected_corr_year
        ].copy()

        annual['datetime'] = pd.to_datetime(
            annual['datetime'], errors='coerce'
        )
        ...
        corr_year_fig.add_trace(
            go.Scatter(
                ...
                name='San Nicolás',
                hovertemplate=(
                    '%{x|%d/%m/%Y}<br>Δ San Nicolás: '
                    '%{y:+.2f} m<extra></extra>'
                ),
            )
        )

        corr_year_fig.add_hline(y=0, line_width=1, line_dash='dot')
        ...
        st.plotly_chart(corr_year_fig, use_container_width=True)
        ...
        comparison_source = hydro_dataset[
            ['datetime', 'nivel_corrientes', 'nivel_san_nicolas']
        ].copy()

        comparison_source['datetime'] = pd.to_datetime(
            comparison_source['datetime'], errors='coerce'
        )

        comparison_source['nivel_corrientes'] = pd.to_numeric(
            comparison_source['nivel_corrientes'], errors='coerce'
        )

        comparison_source['nivel_san_nicolas'] = pd.to_numeric(
            comparison_source['nivel_san_nicolas'], errors='coerce'
        )
        ...
        if years_same_day:
            ...
            if not paired_abs.empty:
                ...
                same_day_corr = paired_abs['nivel_corrientes'].corr(
                ...
                if pd.notna(same_day_corr):
                    st.caption(
                        ...
                        'Este valor es descriptivo y no incorpora demora '
                        'de propagación.'
                    )

            st.caption(
                'Comparación lectura contra lectura: Corrientes y '
                'San Nicolás se muestran exactamente en la misma fecha, '
                'sin trasladar Corrientes 7 días ni aplicar ningún otro '
                'retardo. El selector permite revisar la relación año '
                'por año. Se mantienen dos ejes verticales porque cada '
                'estación posee un cero hidrométrico diferente.'
            )
        else:
            st.info(
                'No hay años con suficientes lecturas coincidentes '
                'de Corrientes y San Nicolás para mostrar la comparación '
                'de alturas en la misma fecha.'
            )
    ...
    for col in ['Correlación', 'Respuesta SN/Corrientes [m/m]']:
        if col in table_display.columns:
            table_display[col] = pd.to_numeric(
                table_display[col], errors='coerce'
            ).round(3)
...
coverage_table['Estado'] = coverage_table['Activo'].map(
    {True: '✅', False: '❌'}
)
...
with m1:
    st.metric(
        ...
        (
            ...
            if np.isfinite(safe_float(metrics.get('rmse')))
            else '—'
        ),
    )
...
with m2:
    st.metric(
        ...
        (
            ...
            if np.isfinite(safe_float(metrics.get('mae')))
            else '—'
        ),
    )

with m3:
    st.metric('Filas entrenamiento', str(models.get('training_rows', 0)))

with m4:
    st.metric('Variables', str(models.get('feature_count', 0)))
...
with st.expander('Metodología del pronóstico V11.12'):
    st.markdown(
        """
        **1–15 días**
...
with st.expander('Diagnóstico técnico'):
    ...
    st.write('Última actualización:', st.session_state.get('last_update'))
    ...
    if isinstance(lag_to_sn, pd.DataFrame) and not lag_to_sn.empty:
        st.dataframe(
            lag_to_sn,
            use_container_width=True,
            hide_index=True,
    ...
    if isinstance(corridor_lags, pd.DataFrame) and not corridor_lags.empty:
        st.dataframe(
            corridor_lags,
            use_container_width=True,
            hide_index=True,
    ...
    st.json(
        {
            ...
            'upstream_features': models.get('upstream_feature_count'),
            'flow_features': models.get('flow_feature_count'),
            'rain_features': models.get('rain_feature_count'),
            'hydrology_features': models.get('hydrology_feature_count'),
        }
    )
...
st.caption(
    f'PARANÁ · SAN NICOLÁS · {APP_VERSION} · Modelo experimental de apoyo '
    'al análisis hidrológico. Los escenarios de 30–60 días representan '
    'proyecciones probabilísticas y no sustituyen avisos oficiales de INA.'
)
